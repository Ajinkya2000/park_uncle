...
const DrawerWrapper = (props) => {
    ...
    return (
        <>
            <Drawer backdrop={true} size="xs" placement='left' open={props.open} onClose={() => props.setopen(false)}>
                <Drawer.Header>
                    <Drawer.Title>Parking Details</Drawer.Title>
                    <Drawer.Actions>
                        <Button color="green" appearance="primary" onClick={() => props.setopen(false)}>Book</Button>
                    </Drawer.Actions>
                </Drawer.Header>
                <Drawer.Body>
